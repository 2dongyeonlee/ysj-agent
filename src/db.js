...
export async function getInfoInsightsSince(env, sinceIso, categories) {
  ...
  const { results } = await env.DB.prepare(
    ...
    "AND (i.summary LIKE '%Daily%' OR i.summary LIKE '%경영환경%' " +
    "OR i.summary LIKE '%보고%' OR i.summary LIKE '%동향%' OR i.summary LIKE '%브리핑%' " +
    "OR EXISTS (SELECT 1 FROM messages m3 WHERE m3.message_id = CASE WHEN instr(i.source_ref,'#')>0 THEN substr(i.source_ref,1,instr(i.source_ref,'#')-1) ELSE i.source_ref END AND (m3.text LIKE '%<Daily>%' OR m3.text LIKE '%보고드립니다%' OR m3.text LIKE '%경영환경%')) " +
