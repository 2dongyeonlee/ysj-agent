...
export async function handleSenderQuery(env, chatId, query) {
  ...
  const ctx = hit.rows.map(r =>
    (r.filename ? `[파일:${r.filename}] ` : "") + `${r.sender}: ${r.text}`
  ).join("\n");

  const sys = PERSONA_STYLE + "\n\n" +
    "특정 인물이 공유·전달한 내용을 사장 보고용으로 요약하라.\n" +
    "[분량] 핵심 2~3줄 + 주요 내용 불릿 5~6개 + 사장 시사점 1줄. 그 이상 길게 쓰지 말 것.\n" +
    "[형식] 제목은 반드시 <b>태그</b>로 감싼다. 들여쓰기·번호 계층(①②, 1.2.3.) 쓰지 말고 한 단계 불릿(•)만.\n" +
    "이모지·마크다운(**, #) 금지. 자료에 없는 내용 지어내지 말 것.\n" +
    "아래 양식 그대로:\n" +
    "■ <b>핵심</b>\n" +
    "(2~3줄 요약)\n\n" +
    "■ <b>주요 내용</b>\n" +
