...
import { sendMessage, sendDocument, sendDocumentBytes, answerCallbackQuery } from "./telegram.js";
...
// 브리핑 인라인 버튼 분기. 기존 브리핑 함수를 그대로 재사용하고, 스텁 2개만 안내 응답.
export async function handleCallback(env, cq) {
  // 스피너 해제를 먼저 — 브리핑 생성(수십 초)이 끝나길 기다리면 버튼이 계속 로딩 상태로 남는다.
  await answerCallbackQuery(env, cq.id).catch(() => {});
...
// Cloudflare Workers 의 fetch/scheduled 진입점은 로컬 서버에서 쓰지 않는다.
// 대신 telegramPoll.js 가 route()/handleCallback() 을, scheduler.js 가
// runVoiceQueue 등 큐 처리 함수를 직접 호출한다(entry point 만 교체, 로직은 동일).

export async function route(env, msg) {
  ...
  // 대외정보 요청 -> info
  if ((isDM || isMentioned) &&
      /^(대외\s*정보|대외\s*동향|정보\s*브리핑|대외\s*브리핑|오늘\s*(대외|정보|뉴스))/.test(t)) {
    return runInfoBriefing(env, chatId, 2);
  }
  // 주요 업무 요청 -> brief
  if ((isDM || isMentioned) &&
      /^(브리핑|업무\s*브리핑|주요\s*업무|오늘\s*업무|주요\s*(내용|사항)\s*(브리핑|요약|정리)?|업무\s*요약|전체\s*요약|요약\s*(해줘|해주세요|좀)?)$/.test(t)) {
