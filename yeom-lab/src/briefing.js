...
async function sendLongMessage(env, chatId, text, extra) {
  ...
  for (let i = 0; i < parts.length; i++) {
    await sendMessage(env, chatId, parts[i], i === parts.length - 1 ? extra : undefined);
  }
}
...
export async function runBrief(env, chatId, extraDays = 0) {
  ...
  const internalRows = ((await getInsightsSince(env, sinceDaysIso(7 + extraDays), { projectEmpty: true })) || [])
